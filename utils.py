...
def currency_rates(valute_id):
    ...
    if content.find(valute_id.upper()) != -1:  # Чтобы функция принимала аргумент в любом регистре используется метод
        # строки upper
        number_of_occurrences = content.count('<CharCode>', 0, content.index(valute_id.upper()))
